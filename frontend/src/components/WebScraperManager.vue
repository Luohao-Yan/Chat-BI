...
const dataTableHeaders = computed(() => {
  ...
  const headers = [{ title: '序号', key: 'index' }]
  Object.keys(firstItem).forEach(key => {
    headers.push({ title: key, key })
  })
  return headers
})
...
const loadScrapeSources = async () => {
  ...
  try {
    ...
    scrapeSources.value = response.data.sources || []
  } catch (error) {
...
const loadCrawlResults = async () => {
  try {
    ...
    crawlResults.value = response.data.results || []
  } catch (error) {
...
const viewCrawledData = async (result: CrawlResult) => {
  ...
  try {
    ...
    crawledDataItems.value = [
      { title: '示例标题1', date: '2025-01-20', content: '示例内容1' },
      { title: '示例标题2', date: '2025-01-20', content: '示例内容2' }
    ]
    showDataViewDialog.value = true
  }
}
